  ...
  return (
    <Layout title="Executive Profile">
      <div className="p-5 pb-24 space-y-10 max-w-2xl mx-auto relative">
        ...
        <div className="bg-slate-950 rounded-[2.5rem] p-6 text-white relative overflow-hidden shadow-xl group border border-white/5">
          ...
          <div className="relative z-10 flex flex-col items-center">
            <div className="relative group mb-5">
              <div className="w-24 h-24 bg-slate-900 rounded-3xl flex items-center justify-center text-blue-500 border-2 border-white/10 shadow-xl relative z-10 overflow-hidden group-hover:scale-105 transition-transform duration-700">
                {newPhotoUrl ? (
            ...
            {!isEditing ? (
              <div className="text-center space-y-3">
                <div className="space-y-0.5">
                  <h2 className="text-xl font-display font-black tracking-tighter uppercase italic">{profile?.displayName}</h2>
                  <p className="text-slate-500 text-[8px] font-black uppercase tracking-[0.2em] font-sans">{profile?.email}</p>
                </div>
                
                <div className="flex justify-center gap-3">
                  <div className="flex items-center gap-1.5 bg-blue-600 text-white text-[8px] font-black uppercase px-4 py-1.5 rounded-xl tracking-[0.1em] shadow-[0_0_15px_rgba(37,99,235,0.4)]">
                    <Star size={10} className="fill-white" />
                    Tier {profile?.level || 1}
                  </div>
                  <div className="flex items-center gap-1.5 bg-white/10 text-white text-[8px] font-black uppercase px-4 py-1.5 rounded-xl tracking-[0.1em] border border-white/10 backdrop-blur-md">
                ...
                <div className="pt-2">
                  <button 
                    ...
                    className="px-6 py-2 bg-white/5 border border-white/10 rounded-xl text-slate-300 text-[8px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-1.5 hover:bg-white/10 hover:text-white transition-all backdrop-blur-md"
                  >
                    <Edit2 size={10} /> Customize Identity
                  </button>
                </div>
              </div>
            ) : (
              <div className="w-full max-w-sm space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 pl-2">First Domain</p>
                    <input 
                      type="text" 
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-sm font-bold text-white focus:ring-2 focus:ring-blue-600/50 outline-none"
                      value={newFirstName}
                      onChange={(e) => setNewFirstName(e.target.value)}
                    />
                  </div>
                  <div className="space-y-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 pl-2">Last Domain</p>
                    <input 
                      type="text" 
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-sm font-bold text-white focus:ring-2 focus:ring-blue-600/50 outline-none"
                      value={newLastName}
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    disabled={saving}
                    className="flex-1 bg-blue-600 text-white font-black py-4 rounded-2xl shadow-xl shadow-blue-900/40 flex items-center justify-center gap-3 active:scale-[0.98] transition-all uppercase tracking-widest text-[10px]"
                  >
                    {saving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Check size={18} /> Update Core</>}
                  </button>
                  <button 
                    onClick={() => setIsEditing(false)}
                    className="px-8 bg-white/5 text-white font-black py-4 rounded-2xl flex items-center justify-center active:scale-[0.98] transition-all border border-white/10"
                  >
                    <X size={18} />
                  </button>
                </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm text-center group hover:border-blue-100 transition-colors">
            <div className="w-8 h-8 bg-orange-50 text-orange-500 rounded-lg flex items-center justify-center mx-auto mb-2 transition-transform group-hover:scale-110">
              <Flame size={16} className="fill-orange-500" />
            </div>
            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Active</p>
            <p className="font-display font-black text-slate-900 text-sm italic">{profile?.streak || 0} Days</p>
          </div>
          <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm text-center group hover:border-blue-100 transition-colors">
            <div className="w-8 h-8 bg-blue-50 text-blue-500 rounded-lg flex items-center justify-center mx-auto mb-2 transition-transform group-hover:scale-110">
              <Settings size={16} className="animate-spin-slow" />
            </div>
            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Growth</p>
            <p className="font-display font-black text-slate-900 text-sm italic">+{((profile?.xp || 0) > 999 ? (profile?.xp! / 1000).toFixed(1) + 'k' : profile?.xp || 0)}</p>
          </div>
          <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm text-center group hover:border-blue-100 transition-colors">
            <div className="w-8 h-8 bg-purple-50 text-purple-500 rounded-lg flex items-center justify-center mx-auto mb-2 transition-transform group-hover:scale-110">
              <Award size={16} className="fill-purple-500" />
            </div>
            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Rank</p>
            <p className="font-display font-black text-slate-900 text-sm italic">{profile?.badges?.length || 1} Elite</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-950 text-white rounded-[2rem] p-6 border border-white/5 shadow-xl relative overflow-hidden group">
            {/* Ambient gradients */}
            ...
            <div className="relative z-10 space-y-6">
              <div className="grid grid-cols-2 gap-4 pb-4 border-b border-white/10">
                <div className="space-y-1">
                  <p className="text-[9px] font-black uppercase text-slate-500 tracking-wider">Total Terminated (Withdrawn)</p>
                  <p className="font-display font-black text-2xl text-emerald-400 tracking-tighter italic">
                    {loadingWithdrawals ? (
                ...
                <div className="space-y-1 border-l border-white/10 pl-4">
                  ...
                  <p className="font-display font-black text-2xl text-amber-400 tracking-tighter italic animate-pulse">
                    {loadingWithdrawals ? (
                      <span className="opacity-50 text-slate-500">₦...</span>
                    ) : (
                ...
                {loadingWithdrawals ? (
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto no-scrollbar">
                    {activeRequests.map((req, idx) => (
                      <div key={req.id || idx} className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between hover:border-white/20 transition-all duration-300">
                        <div className="flex items-center gap-4">
                          <div className="w-9 h-9 bg-amber-500/10 text-amber-400 rounded-xl flex items-center justify-center border border-amber-500/20 relative">
                            <span className="w-2 h-2 rounded-full bg-amber-400 absolute top-1 right-1 animate-ping" />
                            <Clock size={16} />
                          </div>
                          <div>
                            <p className="text-sm font-display font-black tracking-tight text-white italic">₦{req.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <span className="text-[8px] font-black uppercase tracking-widest text-amber-400 bg-amber-400/10 px-3 py-1 rounded-full border border-amber-400/20 italic">
                            {req.status}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="bg-white/5 border border-white/5 rounded-2xl py-5 px-3 text-center">
                    <p className="text-[9px] font-black tracking-widest uppercase text-slate-500 leading-none italic">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden p-1.5">
            <div className="grid grid-cols-1 divide-y divide-slate-50">
              {menuItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-9 h-9 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-slate-950 group-hover:text-white transition-all duration-500">
                        <Icon size={20} />
                      </div>
                      <span className="font-display font-black text-slate-900 text-xs uppercase tracking-tight italic group-hover:text-blue-600 transition-colors">{item.label}</span>
                    </div>
                    <div className="w-6 h-6 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-all opacity-0 group-hover:opacity-100">
          ...
          <div className="bg-slate-950 text-white rounded-[2rem] p-6 border border-white/5 shadow-xl relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-display font-black text-sm uppercase tracking-wider text-white italic">Everyday Masterclass</h4>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">Siphon Nigerian digital wealth blueprints directly to your inbox</p>
                </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5 relative overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="flex h-2 w-2 relative">
                        ...
                        <span className={`relative inline-flex rounded-full h-2 w-2 ${profile?.dailyEmailEnabled !== false && profile?.dailyPushEnabled !== false ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
                      </span>
                  ...
                  <button
                    ...
                    className={`font-black px-5 py-3 rounded-xl uppercase tracking-wider text-[9px] flex items-center justify-center gap-2 transition-all cursor-pointer active:scale-95 text-center ${
                      profile?.dailyEmailEnabled !== false && profile?.dailyPushEnabled !== false
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <button
                  ...
                  className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between hover:border-white/20 hover:bg-white/10 transition-all duration-300 text-left cursor-pointer"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-blue-600/20 text-blue-400 rounded-xl flex items-center justify-center">
                      <Mail size={16} />
                    </div>
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-wider text-white">Daily Emails</p>
                      <p className="text-[8px] text-slate-400 font-bold uppercase mt-0.5">Strategy Digests</p>
                    </div>
                  </div>
                  <div className={`w-8 h-5 rounded-full p-1 transition-colors duration-300 ${profile?.dailyEmailEnabled !== false ? 'bg-blue-600' : 'bg-slate-800'}`}>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-purple-600/20 text-purple-400 rounded-xl flex items-center justify-center">
                      <Smartphone size={16} />
                    </div>
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-wider text-white">Phone Alerts</p>
                      <p className="text-[8px] text-slate-400 font-bold uppercase mt-0.5">Dopamine Hooks</p>
                    </div>
                  </div>
                  <div className={`w-8 h-5 rounded-full p-1 transition-colors duration-300 ${profile?.dailyPushEnabled !== false ? 'bg-purple-600' : 'bg-slate-800'}`}>
