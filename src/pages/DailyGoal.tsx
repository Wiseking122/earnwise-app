        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-display font-black text-slate-800 text-sm tracking-tight">Your Daily Goal</h4>
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-100 p-1 rounded-2xl">
              <button 
                onClick={() => handleAdjustTarget(-1)}
                disabled={dailyTarget <= 1}
                className="w-7 h-7 bg-white rounded-xl flex items-center justify-center border border-slate-100 hover:border-slate-200 text-slate-600 active:scale-90 disabled:opacity-40 transition-all cursor-pointer shadow-xs"
                title="Decrease Daily Target"
              >
                <Minus size={12} strokeWidth={3} />
              </button>
              <span className="text-xs font-black text-slate-800 px-2 min-w-[20px] text-center select-none">
                {dailyTarget}
              </span>
              <button 
                onClick={() => handleAdjustTarget(1)}
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              <div>
                <span className={`font-display font-black text-3xl leading-none tracking-tight ${
                ...
                <span className={`text-[10px] font-black uppercase tracking-wider ml-2.5 px-2.5 py-0.5 rounded-full inline-block ${
                  isCompleted 
            ...
            <div className="h-4 bg-slate-50 border border-slate-100 rounded-2xl overflow-hidden relative p-0.5">
              <motion.div 
                initial={{ width: 0 }}
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider mr-1.5 select-none">Presets:</span>
            {[3, 5, 8, 10, 15].map((preset) => (
              <button
                key={preset}
                onClick={() => handleSetTargetDirect(preset)}
                className={`text-[10px] font-black px-3 py-1 rounded-full cursor-pointer border transition-all active:scale-95 ${
                  dailyTarget === preset
          ...
          <div className="pt-1.5 border-t border-slate-100 min-h-[36px] flex items-center justify-between text-[11px] leading-relaxed">
            <div className="flex items-center gap-2 text-slate-500 font-medium">
              {isCompleted ? (
                <>
                  <CheckCircle2 size={13} className="text-emerald-500" />
                  <span>Congrats! You reached your daily goal. Extra task completions yield additional rewards.</span>
                </>
              ) : (
                <>
                  <Sparkles size={11} className="text-orange-400 animate-pulse" />
                  <span>Complete {dailyTarget - completionsToday} more task{dailyTarget - completionsToday === 1 ? '' : 's'} to hit your goal today!</span>
                </>
              )}
            </div>
          </div>
